...
void computeHash(const string& hashProgName)
{
    ...
    if(read(parentToChildPipe[READ_END], fileNameRecv, strlen(fileNameRecv)))
	...
	*/
	...
	if (fp == NULL)
	{
		fprintf("Couldn't start command: %s",cmdLine);
		exit(1);
	}

	fgets(hashValue, HASH_VALUE_LENGTH, fp);
	...
	/* TODO: Send a string to the parent 
	 ...
	 .
	*/
	write(childToParentPipe[WRITE_END], hashValue, HASH_VALUE_LENGTH + 1);
...
void parentFunc(const string& hashProgName)
{
	...
	write(parentToChildPipe[WRITE_END], fileName, strlen(fileName) + 1);
	 ...
	 /* TODO: Read the string sent by the child
	  ...
	  .
	  */
	  read(childToParentPipe[0],hashValue, HASH_VALUE_LENGTH);
...
int main(int argc, char** argv)
{
	...
	for (int hashAlgNum = 0; hashAlgNum < HASH_PROG_ARRAY_SIZE; ++hashAlgNum)
	{
		...
		/** TODO: create two pipes **/
  		int p2c = pipe(parentToChildPipe);
        if(p2c == -1){
            fprintf("ParentToChildPipe failed\n");
            return 1;
        }

        int c2p = pipe(childToParentPipe);
        if(c2p == -1){
            fprintf("ChildToParentPipe failed\n");
            return 1;
        }
